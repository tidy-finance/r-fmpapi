...
#'
#' This function prompts the user to input their Financial Modeling Prep (FMP) API key
#' and saves it to a `.Renviron` file, either in the project directory or the home directory.
#' It also gives the user the option to add `.Renviron` to `.gitignore` for security purposes.
#'
#' @details The function checks if an API key already exists in the specified `.Renviron` file.
#' If a key is found, the user is prompted whether they want to overwrite it.
#' The function allows saving the key either in the project's `.Renviron` file or in the
#' user's home `.Renviron` file. It also recommends adding `.Renviron` to `.gitignore`
#' to avoid accidentally sharing the API key in version control.
#'
#' @return Invisibly returns `TRUE` after saving the key or aborting the operation.
#' The function will read the `.Renviron` file after saving, allowing the environment
#' variables to be immediately available. A restart of the R session is recommended.
#'
#' @export
#'
set_fmp_api_key <- function() {
  ...
  if (fmp_api_key_exists) {
    overwrite_choice <- readline(
      prompt = "API key already exist. Do you want to overwrite it? Enter 'yes' or 'no': "
    )
  ...
  cli::cli_inform(
    "FMP API key has been set and saved in {.file .Renviron} in your {.path {location_choice}} directory. Please restart your R session to load the new environment."
  )
}
