#' Get balance sheet statements from the Financial Modeling Prep API
#'
#' This function retrieves the balance sheet statements for a specified stock symbol from the Financial Modeling Prep (FMP) API.
#'
...
#'
#' @details The function sends a request to the FMP API to fetch balance sheet statements for the provided stock symbol.
...
#' }
#'
get_balance_sheet_statements <- function(
    symbol,
    period = "annual",
    limit = 100
) {
  ...
  resource <- paste0("balance-sheet-statement/", symbol)
  balance_sheet_statements_raw <- perform_request(resource, period = period, limit = limit)
